...
24.问题：(1)富文本编辑器使用时，数据变化无法更新视图？视图变化却可以更新数据?
...
27.解决父组件将子组件作为弹窗调用只执行一次created的问题: 产生原因：你把子组件写在父组件的template里，当父组件渲染的时候，子组件也渲染完成了，以及子组件的生命周期也走完了。 解决方案：(1)父组件通过v-if调用子组件(2)父组件直接通过refs等方法控制子组件的数据。

28.课程的内容管理中，课时的编辑处理中，为什么数据存储中存在数据，而视图却不显示数据??---------->>自己尝试的方法：与后台接口数据比对，一样的放一起，不一样的放在另一边。即：ruleForm: { 此处存放接口无关数据，form:{ 此处存放接口有关的数据 } }--否则容易乱

29.*****element ui 嵌套对象的验证问题？？ ----》二级对象造成的问题，如果这样绑定的话，data里声明的就是二级对象的形式，故而prop中也要以二级对象的形式属性，rules中也是如此，且rules中需要“”引起来。
